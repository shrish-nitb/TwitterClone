...
func (m *TweetMutation) SetUser(s string) {
	m.user = &s
}

// User returns the value of the "user" field in the mutation.
func (m *TweetMutation) User() (r string, exists bool) {
	v := m.user
	if v == nil {
		return
	}
...
// OldUser returns the old "user" field's value of the Tweet entity.
// If the Tweet object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *TweetMutation) OldUser(ctx context.Context) (v string, err error) {
	if !m.op.Is(OpUpdateOne) {
...
func (m *TweetMutation) Fields() []string {
	...
	if m.user != nil {
		fields = append(fields, tweet.FieldUser)
	}
	return fields
}
